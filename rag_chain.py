...
class RAGChain:
    ...
    def _generate_answer(
        ...
        if is_japanese:
            system_message = """あなたは日本語のみで回答するAIアシスタントです。
...
指示:
1. 提供されたソースの情報のみを使用
2. ソース番号を引用（例：「ソース1によると」）
3. 簡潔かつ包括的に回答

例:
質問: 次期POSシステムとは？
回答: 次期POSシステムは、クラウド環境上に構築された基盤システムです。（ソース1より）"""

            user_message = f"""以下は文書からの情報です:

{context}

質問: {query}

【重要】必ず日本語のみで回答してください。英語は絶対に使わないでください。
上記の情報に基づいて回答してください。"""
